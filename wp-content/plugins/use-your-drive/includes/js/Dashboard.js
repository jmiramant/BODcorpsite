  ...
  $.widget("cp.UseyourDriveDashboard", {
    ...
    loadChartData: function () {
      ...
      $.ajax({
        ...
        data: {
          ...
          type: 'activities',
          start: moment(self.chart_datepicker_from.datepicker("getDate")).format('YYYY-MM-DD'),
          end: moment(self.chart_datepicker_to.datepicker("getDate")).format('YYYY-MM-DD'),
    ...
    loadTopDownloads: function () {
      ...
      self.options.$container_top_downloads.DataTable({
        ...
        "processing": true,
        "serverSide": true,
        "ajax": self.options.ajax_url + '?action=useyourdrive-event-stats&type=topdownloads&_ajax_nonce=' + self.options.admin_nonce
      });
    },
    loadTopUsers: function () {
      ...
      self.options.$container_top_users.DataTable({
        ...
        "processing": true,
        "serverSide": true,
        "ajax": self.options.ajax_url + '?action=useyourdrive-event-stats&type=topusers&_ajax_nonce=' + self.options.admin_nonce
      });
    },
    loadFullLog: function () {
      var self = this;

      self.options.$container_full_log.DataTable({
        dom: 'fBrtip<"clear">',
        "language": {
        ...
        "processing": true,
        "serverSide": true,
        "ajax": self.options.ajax_url + '?action=useyourdrive-event-stats&type=full-log&_ajax_nonce=' + self.options.admin_nonce
      });
    },
    ...
    viewEntryDetails: function (id) {
      var self = this;

      self.openModal('entry', id);
    },

    viewUserDetails: function (id) {
      var self = this;

      self.openModal('user', id);

    },

    openModal: function (detail, id) {
      ...
      var modalbuttons = '';
      modalbuttons += '<button class="simple-button blue useyourdrive-modal-cancel-btn" data-action="cancel" type="button" onclick="modal_action.close();" title="' + self.options.str_close_title + '" >' + self.options.str_close_title + '</button>';
